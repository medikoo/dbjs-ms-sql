...
module.exports = function self(connection, tableName, def, records) {
	if (!records.length) {
		def.resolve();
		return;
	}
	if (promises[tableName]) {
		promises[tableName].finally(self.bind(this, connection, tableName, def, records));
		return;
	}
	promises[tableName] = def.promise.finally(function () {
		if (promises[tableName] === def.promise) delete promises[tableName];
	});
